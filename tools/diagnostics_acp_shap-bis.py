...
def _unwrap_feature_union(obj):
    """Retrouve récursivement une FeatureUnion si obj est enveloppé (Pipeline, etc.)."""
    # Déjà une FeatureUnion ?
    if hasattr(obj, "transformer_list"):
        return obj
    # Sinon, si c'est un Pipeline, explorer ses named_steps
    if hasattr(obj, "named_steps"):
        # heuristiques : on teste d'abord 'text', puis quelques alias
...
def _find_b4_text_nodes(features_union):
    """
    Retourne (text_pipeline, pre_union, svd, has_l2) pour la branche 'text' de B4.
    - text_pipeline : Pipeline complet de la branche text
    - pre_union     : FeatureUnion AVANT SVD (détail TF-IDF/stats)
    - svd           : TruncatedSVD entraîné
    - has_l2        : True si un Normalizer L2 suit le SVD
    """
    text_pipeline = dict(features_union.transformer_list).get("text")
    if text_pipeline is None:
        raise RuntimeError("Branche 'text' introuvable dans features.")

    if not hasattr(text_pipeline, "named_steps"):
        raise RuntimeError("Branche 'text' n'est pas un Pipeline; cas non géré.")

    pre_union = text_pipeline.named_steps.get("text")
    if pre_union is None:
        # tenter de la retrouver même si renommée/imbriquée
        pre_union = _unwrap_feature_union(text_pipeline)

    svd = text_pipeline.named_steps.get("svd")
    if svd is None:
        raise RuntimeError("Pas de SVD dans la branche texte de B4.")

    has_l2 = "l2" in text_pipeline.named_steps
    return text_pipeline, pre_union, svd, has_l2

def _block_slices_text_pre(pre_union, df_small):
    """
    Construit les slices des SOUS-BLOCS texte sur l'espace pré-SVD.
    Réutilise votre logique 'B2' (TF-IDF word/char, HasDescription, TitleLength, TextStats, TextStatsPro, Lexicon, etc.)
    """
    # Si vous avez déjà une fonction similaire (ex: _block_slices_text), appelez-la ici :
    return _block_slices_text(pre_union, df_small)  # <-- existant chez vous

def _norm_l2_rows(X):
    """Norme L2 ligne par ligne; retourne (X_normé, norme_par_ligne)."""
    import numpy as np
...
def blocks_b4_per_class_signed_magnitude_backproj_text(args, pipe, df):
    """
    B4 par classe (magnitude signée) avec rétro-projection du texte
    pour retrouver le détail TF-IDF/stats (pré-SVD), combiné avec l'image.
    """
    import numpy as np
    import pandas as pd
    import os

    # 0) Détection des étapes
    features = _detect_features_step(pipe)      # doit déjà exister chez vous
    scaler   = _detect_scaler_step(pipe)        # idem
    clf      = _detect_clf_step(pipe)           # idem
    classes  = _get_classes(clf)                # idem (labels de classes)
    n_classes = len(classes)

    # 1) On restreint les colonnes d'entrée comme ailleurs
    keep_cols = [c for c in ("designation","description","productid","imageid") if c in df.columns]
    if not keep_cols:
        keep_cols = df.columns.tolist()
    df_small = df[keep_cols].copy()

    # 2) Transformations par 'features' pour récupérer :
    #    - X_full : vecteur complet B4 (concat texte_svd_norm + image + …)
    #    - slices 'coarse' pour TEXT_SVD et pour l'image (CNN / pixels / stats)
    X_full = features.transform(df_small)
    coarse_slices = _block_slices_b4_coarse(features, df_small)  # EXISTANT chez vous
    # ex: [("TEXT_SVD", a, b), ("CNN", b, c), ("IMAGE_PIXELS", c, d), ("IMAGE_STATS", d, e), ...]

    # 3) Récup branche texte : pre_union + svd + l2?
    text_pipeline, pre_union, svd, has_l2 = _find_b4_text_nodes(features)

    # 4) Espace texte pré-SVD
    X_text_pre = pre_union.transform(df_small)              # (n_samples, D_pre)
    text_slices_pre = _block_slices_text_pre(pre_union, df_small)  # [("TF-IDF word", s,e), ("HasDescription",...), ...]

    # 5) Espace texte SVD (+ normalisation si besoin) pour calculer alpha_i
    X_text_svd = svd.transform(X_text_pre)                  # (n_samples, K)
    if has_l2:
    ...
    # 6) Localiser tranche TEXT_SVD dans X_full
    text_coarse = [t for t in coarse_slices if t[0].lower().startswith("text")]
    if not text_coarse:
        raise RuntimeError("Tranche TEXT introuvable dans le vecteur B4.")
    _, a_text, b_text = text_coarse[0]
    # Attention : si un StandardScaler global existe, ses scale_ s'appliquent colonne par colonne
    if scaler is not None and hasattr(scaler, "scale_"):
        scale_text = scaler.scale_[a_text:b_text]
    else:
        scale_text = None

    # 7) Poids du classifieur (par classe) restreints à la tranche TEXT_SVD
    #    et éventuelle correction par le scaler (w_scaled = w / scale)
    W_full, bias = _get_linear_weights_and_bias(clf)        # EXISTANT chez vous: (n_classes, n_features)
    W_text_svd = W_full[:, a_text:b_text]                   # (n_classes, K)
    if scale_text is not None:
        W_text_svd_eff = W_text_svd / scale_text[None, :]   # annule l'effet de scale_ du StandardScaler
    else:
        W_text_svd_eff = W_text_svd

    # 8) Rétro-projection des poids vers l'espace pré-SVD : w_pre = C^T @ w_svd_eff
    #    (C = svd.components_, shape: (K, D_pre))
    C = svd.components_                                     # (K, D_pre)
    W_text_pre = (C.T @ W_text_svd_eff.T).T                 # (n_classes, D_pre)

    # 9) Contributions par sous-bloc texte (pré-SVD)
    # score_i_text = alpha_i * (X_text_pre_i · W_text_pre_c)
    # ⇒ contribution bloc b pour i,c : alpha_i * (X_pre_i[b] · W_pre_c[b])
    contrib_text_pos = []   # liste de DataFrame par classe
    contrib_text_neg = []
    n = X_text_pre.shape[0]

    # Pour chaque classe, calculer contributions moyennes signées + magnitudes
    for ci in range(n_classes):
        ...
        for label, s, e in text_slices_pre:
            ...
            w_b = w_pre_c[s:e]                              # (d_b,)
            # produit scalaire par sample
            dot_i = x_b @ w_b                               # (n,)
            signed = alpha * dot_i                          # (n,)
            # magnitude signée = signe(moyenne) * moyenne(|signed|)
            sign = np.sign(signed.mean()) if signed.size else 0.0
            mag  = np.mean(np.abs(signed)) if signed.size else 0.0
            val  = sign * mag
    ...
    # 10) Contributions image (CNN / pixels / stats) — réutilise votre logique existante
    #     (on garde les fonctions que vous utilisez déjà dans la version actuelle)
    img_pos_list, img_neg_list = _compute_b4_image_blocks_signed_mag(
        args, pipe, df_small, features, scaler, clf, coarse_slices
    )

    # 11) Fusionner texte + image pour le plotting par classe
    outdir = os.path.join("results","figures")
    os.makedirs(outdir, exist_ok=True)
    csvdir = os.path.join("results","reports")
    os.makedirs(csvdir, exist_ok=True)

    labels = classes
    for ci, lab in enumerate(labels):
        # build DataFrame : block -> value
        df_text = contrib_text_pos[ci].combine_first(contrib_text_neg[ci]).fillna(0.0)
        df_img  = img_pos_list[ci].combine_first(img_neg_list[ci]).fillna(0.0)
        df_all  = df_text.combine_first(df_img).fillna(0.0)

        # tri
        mode = (args.sort_by or "total").lower()
        if mode == "pos":
            order = df_all.sort_values("value", ascending=False)
        elif mode == "neg":
            order = df_all.sort_values("value", ascending=True)
        elif mode == "net":
            order = df_all.reindex(df_all.index)  # déjà signé
        else:  # total (par magnitude)
            order = df_all.reindex(df_all.index)
            order["abs"] = order["value"].abs()
            order = order.sort_values("abs", ascending=False).drop(columns=["abs"])

        # export CSV par classe
        csv_path = os.path.join(csvdir, f"block_importance_b4_backproj_text_{lab}.csv")
        order.to_csv(csv_path, index=True)

        # plot
        _plot_signed_magnitude_bars(
            series=order["value"],
            title=f"Importance par bloc — B4 (texte rétro-projeté, classe {lab})",
            xlabel="Contribution moyenne au score linéaire (|x·w|)",
            legend=True,
            outfile=os.path.join(outdir, f"block_importance_b4_per_class_signed_mag_backproj_text_{lab}.png")
        )

    # 12) Vue agrégée (toutes classes) optionnelle
    # (moyenne des valeurs par bloc)
    agg = []
    for ci, lab in enumerate(labels):
        df_text = contrib_text_pos[ci].combine_first(contrib_text_neg[ci]).fillna(0.0)
        df_img  = img_pos_list[ci].combine_first(img_neg_list[ci]).fillna(0.0)
    ...
    agg_sorted.drop(columns=["mean"]).to_csv(os.path.join(csvdir, "block_importance_b4_per_class_signed_mag_backproj_text_summary.csv"))
    _plot_signed_magnitude_bars(
        series=agg_sorted["mean"],
        title="Importance par bloc — B4 (texte rétro-projeté, moyenne classes)",
        xlabel="Contribution moyenne au score linéaire (|x·w|)",
        legend=True,
        outfile=os.path.join(outdir, "block_importance_b4_per_class_signed_mag_backproj_text_mean.png")
    )
...
def main():
    ...
    parser.add_argument("--blocks-b4-per-class-signed-mag-backproj-text", action="store_true", help="B4 par classe (magnitude signée) avec rétro-projection du texte avant SVD pour détail TF-IDF/stats.")
    ...
    if args.blocks_b4_per_class_signed_mag_backproj_text: blocks_b4_per_class_signed_magnitude_backproj_text(args, pipe, df)
    return
